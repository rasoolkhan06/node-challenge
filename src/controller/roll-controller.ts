...
import {
  ...
  UpdateStudentRollStateInput,
} from "../interface/student-roll-state.interface";
...
export class RollController {
  ...
  async getRoll(request: Request, response: Response, next: NextFunction) {
    return this.studentRollStateRepository.find({ id: request.params.id });
  }

  async addStudentRollStates(
    request: Request,
    response: Response,
    next: NextFunction
  ) {
  ...
  async updateStudentRollState(
    ...
    this.studentRollStateRepository
      .findOne(params.id)
      .then((studentRollState) => {
